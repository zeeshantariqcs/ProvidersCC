export let isValidNpiNumber = function (npi) {
    var tmp;
    var sum;
    var i;
    var j;
...
export let validateProvider = (firstName, lastName, NpiNumber)=> {
    let errors = "";
    if (firstName !== "" || lastName !== "" || NpiNumber !== "") {

        if (typeof NpiNumber !== "undefined" && NpiNumber !== "") {
            if (!isValidNpiNumber(NpiNumber)) {
                errors += "Please enter a valid NPINumber<br/>";
            }
        }
        let name_regex = new RegExp("^[a-zA-Zàáâäãå????èéêë??ìíîï??òóôöõøùúûü??ÿı??ñç?šÀÁÂÄÃÅ?????ÈÉÊËÌÍÎÏ???ÒÓÔÖÕØÙÚÛÜ??Ÿİ??ÑßÇŒÆ?Š?ğ ,.'-]+$");
